...
config_path = os.path.join(current_dir, 'noise2d.json')
...
for proportion in np.arange(0.1, 1.1, 0.1):
    ...
    config['model_filename'] = f'parameters/noise2d.pth'
    
    temp_config_path = os.path.join(current_dir, f'noise2d.json')
    with open(temp_config_path, 'w') as f:
        json.dump(config, f, indent=4)
    
    print(f"Train the surrogate model. Use proportion = {proportion}...")
    ...
    initial_points = [np.array([1.3, 3.5]), np.array([3.5, 6.0])] 
