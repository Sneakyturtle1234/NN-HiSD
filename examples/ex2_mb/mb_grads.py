...
config_path = os.path.join(current_dir, 'mb_grads.json')
...
for num_points in range(1000, 5500, 1000):
    ...
    for seed in range(40, 50, 1):
        ...
        config['model_filename'] = f'parameters/mbnumbers_grad.pth'

        temp_config_path = os.path.join(current_dir, f'mb_grads.json')
        ...
        for i, w0 in enumerate(initial_points):
            ...
            error_norm = np.linalg.norm(saddle_point - w0)
            print(f"Error of saddle: {error_norm}")
            error_norms.append(error_norm)
            with open(os.path.join(params_dir, f'grads.txt'), 'a') as f:
